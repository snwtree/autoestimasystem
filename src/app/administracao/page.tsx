...
export default function AdministracaoPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (active) setAllSales(remoteSales);
      } catch {
        const stored = window.localStorage.getItem("autoestima-sales");
        if (active && stored) setAllSales(JSON.parse(stored) as Sale[]);
      }
    };
  ...
  const sales = allSales.filter((sale) => sale.date >= periodStart && sale.date <= periodEnd);
  ...
  async function reverseSale(sale: Sale) {
    if (!window.confirm(`Excluir a venda de ${sale.client}?`)) return;
    await deleteSale(sale.id);
    setAllSales((current) => current.filter((item) => item.id !== sale.id));
