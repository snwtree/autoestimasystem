...
import { deleteSaleByAppointmentId, saveSale } from "@/lib/supabase/data";
...
export default function AgendaPage() {
  ...
  async function deleteAppointment(id: string) {
    const appointment = appointments.find((item) => item.id === id);
    if (!appointment || !window.confirm(`Excluir o agendamento de ${appointment.client}?`)) return;
    try {
      await Promise.all([removeAppointment(id), deleteSaleByAppointmentId(id)]);
      setAppointments((current) => current.filter((item) => item.id !== id));
    } catch (error) {
      setFormWarning(`Não foi possível excluir o agendamento: ${describeSupabaseError(error)}`);
    }
  }
